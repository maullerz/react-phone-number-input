  ...
  if (newCountry) {
    ...
    if (phoneDigits[0] === '+') {
      ...
      if (prevCountry) {
        ...
        const defaultValue = getInternationalPhoneNumberPrefix(newCountry, metadata); // If `phoneDigits`'s country calling code part is the same
...
export function onPhoneDigitsChange(phoneDigits, _ref5) {
  ...
  if (international && countryCallingCodeEditable === false) {
    const prefix = getInternationalPhoneNumberPrefix(country, metadata); // The `<input/>` value must start with the country calling code.
...
function convertInternationalPhoneDigitsToNational(input, country, metadata) {
  ...
  if (input.indexOf(getInternationalPhoneNumberPrefix(country, metadata)) === 0) {
    ...
    const formatter = new AsYouType(country, metadata); // Input partial national phone number.
...
export function stripCountryCallingCode(number, country, metadata) {
  ...
  if (country) {
    const countryCallingCodePrefix = '+' + getCountryCallingCode(country, metadata); // If `country` fits the actual `number`.
