...
export default function usePhoneDigits(_ref) {
  ...
  const [prevCountry, setPrevCountry] = useState(country); // This is only used to detect `defaultCountry` property change.

  const [prevDefaultCountry, setPrevDefaultCountry] = useState(defaultCountry); // `phoneDigits` is the `value` passed to the `<input/>`.

  const [phoneDigits, setPhoneDigits] = useState(getInitialPhoneDigits()); // This is only used to detect `value` property changes.

  const [valueForPhoneDigits, setValueForPhoneDigits] = useState(value); // Rerender hack.
