...
function getPreSelectedCountry(_ref) {
  ...
  let country; // If can get country from E.164 phone number
...
function getCountrySelectOptions(_ref2) {
  ...
  countrySelectOptions.sort((a, b) => _compareStrings(a.label, b.label, compareStringsLocales)); // Add the "International" option to the country list (if suitable)
...
function getPhoneDigitsForNewCountry(phoneDigits, _ref3) {
  ...
  if (newCountry) {
    ...
    if (phoneDigits[0] === '+') {
      ...
      if (prevCountry) {
        const newCountryPrefix = (0, _getInternationalPhoneNumberPrefix.default)(newCountry, metadata);
        ...
        const defaultValue = (0, _getInternationalPhoneNumberPrefix.default)(newCountry, metadata); // If `phoneDigits`'s country calling code part is the same
...
function e164(number, country, metadata) {
  ...
  if (number[0] === '+') {
    ...
    const asYouType = new _core.AsYouType(country, metadata);
...
function getCountryForPartialE164Number(partialE164Number, _ref4) {
  ...
  const derived_country = getCountryFromPossiblyIncompleteInternationalPhoneNumber(partialE164Number, metadata); // If a phone number is being input in international form
  // and the country can already be derived from it,
...
function onPhoneDigitsChange(phoneDigits, _ref5) {
  ...
  if (international && countryCallingCodeEditable === false) {
    const prefix = (0, _getInternationalPhoneNumberPrefix.default)(country, metadata); // The `<input/>` value must start with the country calling code.

    if (phoneDigits.indexOf(prefix) !== 0) {
      let value; // If a phone number input is declared as
...
function convertInternationalPhoneDigitsToNational(input, country, metadata) {
  ...
  if (input.indexOf((0, _getInternationalPhoneNumberPrefix.default)(country, metadata)) === 0) {
    ...
    const formatter = new _core.AsYouType(country, metadata); // Input partial national phone number.
...
function getCountryFromPossiblyIncompleteInternationalPhoneNumber(number, metadata) {
  const formatter = new _core.AsYouType(null, metadata);
...
function stripCountryCallingCode(number, country, metadata) {
  ...
  if (country) {
    const countryCallingCodePrefix = '+' + (0, _core.getCountryCallingCode)(country, metadata); // If `country` fits the actual `number`.
  ...
  for (var _i = 0, _Object$keys = Object.keys(metadata.country_calling_codes); _i < _Object$keys.length; _i++) {
    const country_calling_code = _Object$keys[_i];
...
function couldNumberBelongToCountry(number, country, metadata) {
  const intlPhoneNumberPrefix = (0, _getInternationalPhoneNumberPrefix.default)(country, metadata);
  let i = 0;
