...
function sortCountryOptions(options, order) {
  ...
  for (var _iterator = _createForOfIteratorHelperLoose(order), _step; !(_step = _iterator()).done;) {
    ...
    if (element === '|') {
      ...
      const index = options.indexOf(options.filter(option => option.value === countryCode)[0]); // Get the option.

      const option = options[index]; // Remove the option from its default position.
