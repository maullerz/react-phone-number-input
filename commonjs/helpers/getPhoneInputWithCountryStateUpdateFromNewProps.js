...
function getPhoneInputWithCountryStateUpdateFromNewProps(props, prevProps, state) {
  ...
  const _getInitialPhoneDigits = parameters => (0, _phoneInputHelpers.getInitialPhoneDigits)({ ...parameters,
    international,
  ...
  if (newDefaultCountry !== prevDefaultCountry) {
    const isNewDefaultCountrySupported = !newDefaultCountry || (0, _countries.isCountrySupportedWithError)(newDefaultCountry, metadata);

    const noValueHasBeenEnteredByTheUser = // By default, "no value has been entered" means `value` is `undefined`.
  ...
  if (newValue !== prevValue && newValue !== value) {
    ...
    if (newValue) {
      ...
      const supportedCountries = (0, _countries.getSupportedCountries)(countries, metadata);
    ...
    return { ...hasUserSelectedACountryUpdate,
      phoneDigits: _getInitialPhoneDigits({
        phoneNumber,
        value: newValue,
        defaultCountry: newDefaultCountry
