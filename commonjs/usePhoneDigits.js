...
function usePhoneDigits(_ref) {
  ...
  const countryMismatchDetected = (0, _react.useRef)();
  ...
  const [prevCountry, setPrevCountry] = (0, _react.useState)(country); // This is only used to detect `defaultCountry` property change.

  const [prevDefaultCountry, setPrevDefaultCountry] = (0, _react.useState)(defaultCountry); // `phoneDigits` is the `value` passed to the `<input/>`.

  const [phoneDigits, setPhoneDigits] = (0, _react.useState)(getInitialPhoneDigits()); // This is only used to detect `value` property changes.

  const [valueForPhoneDigits, setValueForPhoneDigits] = (0, _react.useState)(value); // Rerender hack.

  const [rerenderTrigger, setRerenderTrigger] = (0, _react.useState)();
  const rerender = (0, _react.useCallback)(() => setRerenderTrigger({}), [setRerenderTrigger]);
  ...
  function getValueForPhoneDigits(phoneDigits) {
    ...
    const asYouType = new _core.AsYouType(country || defaultCountry, metadata);
  ...
  const onSetPhoneDigits = (0, _react.useCallback)(phoneDigits => {
    let value;

    if (country) {
      if (international && withCountryCallingCode) {
        // The `<input/>` value must start with the country calling code.
        const prefix = (0, _getInternationalPhoneNumberPrefix.default)(country, metadata);
...
function getPhoneDigitsForValue(value, country, international, withCountryCallingCode, defaultCountry, useNationalFormatForDefaultCountryValue, metadata, onCountryMismatch) {
  ...
  const asYouType = new _core.AsYouType(undefined, metadata);
